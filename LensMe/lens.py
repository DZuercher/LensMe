...
class nfw_halo_lens:
    def __init__(self, M_halo=200., c_halo=3., z_halo=0.5, z_source=1.,
                 nx=1020, ny=1020, frac_pos_x=0.5, frac_pos_y=0.5):
        """
        NFW halo lens.
        The nfw_halo_lens class is heavily based on
        `gravity-game <https://github.com/mdlreyes/gravity-game>`_
        """

        self.nx = nx
        self.ny = ny
        # set angular resolution o 0.1 arcmin
        self.dx = 0.1 / 60. / 180. * np.pi
        ...
        self.d_ls = cosmology._comoving_distance_z1z2(
            self.z_halo, self.z_source).value

        self.rho_c = cosmology.critical_density(
            self.z_halo).value * 1000. * mpc ** 3. / solar
        self.r_s = (3. / 4. * M / self.rho_c / 500. / np.pi
                    / self.c ** 3.) ** (1. / 3.)
        self.rho_0 = self.rho_c * 500. / 3. * self.c ** 3. \
            / (np.log(1. + self.c) - self.c / (1. + self.c))
        self.r_500 = self.c*self.r_s
        self.R = 5. * self.r_500

        self.gamma = G / c_light ** 2. * solar / mpc
        ...
        self.d_ls_ad = cosmology.angular_diameter_distance_z1z2(
            self.z_halo, self.z_source).value
        self.Sigma_c = 1. / (4. * np.pi * self.d_l_ad *
                             self.d_ls_ad * self.gamma) * self.d_s_ad
        ...
        for i in tqdm.tqdm(range(ny)):
            for j in range(nx):
                ...
                (self.deflection_x[i, j], self.deflection_y[i, j]) = \
                    self.calc_deflection_field(i, j)

        # angular positions of the pixels
        self.x, self.y = np.meshgrid(
            np.arange(0, self.nx) * self.dx,
            np.arange(0, self.ny) * self.dy)
    ...
    def calc_deflection_field(self, i, j):
        """
        Calculate the deflection angle in x and y
        direction given a pixel id i,j
        """
        ...
        if (0. < r < R) & np.isclose(r, 1.):
            ...
            f = (-2. * cmath.atan(sqrt_Rsq_rsq / sqrt_one_rsq) / sqrt_one_rsq
                 + 2. * cmath.atan(sqrt_Rsq_rsq / (R * sqrt_one_rsq))
                 / sqrt_one_rsq + cmath.log(R + sqrt_Rsq_rsq) -
                 cmath.log(R - sqrt_Rsq_rsq)
                 - 2. * sqrt_Rsq_rsq / (1. + R) - 2. * (-R + (1. + R)
                 * cmath.log(1. + R)) / (1. + R)) / r ** 2.
            f = f.real
        elif r == 0.:
            f = 0.
        else:
        ...
        alpha_x = alpha_x * self.d_ls_ad / self.d_s_ad * 8. \
            * np.pi * self.rho_0 * self.r_s**2. * self.gamma
        alpha_y = alpha_y * self.d_ls_ad / self.d_s_ad * 8. \
            * np.pi * self.rho_0 * self.r_s**2. * self.gamma
    ...
    def __call__(self, img, reshape=True):
        ...
        image_lensed[:, :, 0] = interpolate.RectBivariateSpline(
            np.arange(0, self.ny) * self.dy, np.arange(
                0, self.nx) * self.dx, img[:, :, 0]).ev(
                    lys, lxs).reshape([self.ny, self.nx])
        image_lensed[:, :, 1] = interpolate.RectBivariateSpline(
            np.arange(0, self.ny) * self.dy, np.arange(
                0, self.nx) * self.dx, img[:, :, 1]).ev(
                    lys, lxs).reshape([self.ny, self.nx])
        image_lensed[:, :, 2] = interpolate.RectBivariateSpline(
            np.arange(0, self.ny) * self.dy, np.arange(
                0, self.nx) * self.dx, img[:, :, 2]).ev(
                    lys, lxs).reshape([self.ny, self.nx])
        return image_lensed
